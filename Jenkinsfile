...
pipeline {
  ...
  environment {
    ...
    //NEXUS_URL = "http://18.141.239.171:8081"
    ...
    stage('SonarQube Inspection') {
        steps {
            withSonarQubeEnv('SonarQube') { 
                ...
                -Dsonar.host.url=http://47.129.217.83:9000\
    ...
    stage("Nexus Artifact Uploader"){
        steps{
           nexusArtifactUploader(
              nexusVersion: 'nexus3',
              protocol: 'http',
              nexusUrl: 'http://18.141.239.171:8081',
              groupId: 'webapp',
  ...
  post {
    always {
        ...
        slackSend channel: '#bandaru-jenkins-cicd-pipeline-alerts', //update and provide your channel name
